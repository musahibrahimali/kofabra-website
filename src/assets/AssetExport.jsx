/* Image exports */
export {default as Image1} from './images/svg-1.svg';
export {default as Image2} from './images/svg-2.svg';
export {default as Image3} from './images/svg-3.svg';
export {default as Image4} from './images/svg-4.svg';
export {default as Image5} from './images/svg-5.svg';
export {default as Image6} from './images/svg-6.svg';
export {default as Image7} from './images/svg-7.svg';
export {default as Image8} from './images/svg-8.svg';
export {default as loginImage} from './images/login.svg';

/* Logo */
export {default as Logo} from './images/brand-image.png';
export {default as LogoOne} from './images/brand-image-1.png';
export {default as LogoTwo} from './images/brand-image-2.png';